...
ヘッダーの「リポジトリから開く」から、ローカルにファイルを置かずに
`azumag/areNews` の `episodes/*/script.json` を直接選んで読み込めます。

- 一覧取得は GitHub の Git Trees API（`git/trees/main?recursive=1`）を1回呼ぶだけです
  （`episodes/`配下を1件ずつ調べて回数を増やすことはしません）
- 本文取得は `raw.githubusercontent.com` から。どちらも認証なしで動きます
  （このリポジトリがpublicであることが前提です）
- 台本の検証はローカルファイルを開く場合と同じRust側のスキーマ検証を通ります
- 「再読込」と「前回開いた台本を起動時に開く」は、リポジトリから開いた場合も
  同じ場所（`episodes/<id>/script.json`）へ再度アクセスします
- 対象リポジトリは `src/lib/githubRepo.ts` の先頭でハードコードしています
  （このアプリ専用の配線のため、設定画面からは変更できません）
...
設定画面の「上の既定話者を台本の指定より優先する」をONにすると、**アプリ設定で選んだキャラクターに限り**
この優先順位が逆転し、台本側の指定（台詞単位・エピソード既定とも）を無視してアプリ設定の声を使います。
アプリ設定で声を選んでいないキャラクターは通常通り台本側の指定に従います。台本ファイルを書き換えずに
声だけ差し替えたい場合のためのオプションです（既定はOFF、Issue記載の優先順位どおり）。
...
- [ ] OBSの「アプリケーション音声キャプチャ（Windows 10+）」で `areNews Controller` を
      選択すると、読み上げ音声を録れる（音声はWebViewではなく本体プロセスから
      ネイティブ再生している — `src-tauri/src/audio_player.rs` 参照）
